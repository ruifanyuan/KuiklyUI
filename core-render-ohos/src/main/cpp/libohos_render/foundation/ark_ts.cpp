...
napi_value ArkTS::Call(napi_value callback, std::vector<napi_value> args, napi_value this_object) {
    ...
    // KLOG_DEBUG("ArkTS") << "ArkTS::Call params, env:" << env_
    //    << ", obj:" << this_object
    //    << ", callback:" << callback
    //    << ", args:" << args.size();
    ...
    // KLOG_DEBUG("ArkTS") << "ArkTS::Call params, status:" << status;
...
napi_value ArkTS::CreateInt8Array(const void *external_data, size_t byte_length) {
    napi_value arraybuffer, result;
    void *data;
    auto status = napi_create_arraybuffer(env_, byte_length, &data, &arraybuffer);

    if (status == napi_ok && data != nullptr && external_data != nullptr) {
        memcpy(data, external_data, byte_length);
    }

    status = napi_create_typedarray(env_, napi_int8_array, byte_length, arraybuffer, 0, &result);
...
std::vector<napi_value> ArkTS::GetCallbackArgs(napi_callback_info info, void **data) {
    size_t argc;
    napi_get_cb_info(env_, info, &argc, nullptr, nullptr, nullptr);
    return GetCallbackArgs(info, argc, data);
}

std::vector<napi_value> ArkTS::GetCallbackArgs(napi_callback_info info, size_t args_count, void **data) {
    size_t argc = args_count;
    std::vector<napi_value> args(args_count, nullptr);
    napi_get_cb_info(env_, info, &argc, args.data(), nullptr, data);
    return args;
}
...
bool ArkTS::IsTypedArray(napi_value array) {
    napi_status status;
    bool result = false;
    status = napi_is_typedarray(env_, array, &result);
    this->MaybeThrowFromStatus(status, "Failed to is typedarray");
    return result;
}
...
std::string ArkTS::GetString(napi_value value) {
    ...
    status = napi_get_value_string_utf8(env_, value, reinterpret_cast<char *>(buffer.data()), length + 1, &length);
...
void ArkTS::MaybeThrowFromStatus(napi_status status, const char *message) {
    if (status != napi_ok) {
        ...
        // KLOG_ERROR("ArkTS") << "ArkTS::MaybeThrowFromStatus, msg:" << full_msg;
...
void ArkTS::PrintValue(napi_value value) {
    ...
    switch (type) {
    case napi_undefined: {
        // KLOG_INFO("ArkTS") << "ArkTS::PrintValue: undefined";
    } break;
    case napi_null: {
        // KLOG_INFO("ArkTS") << "ArkTS::PrintValue: null";
    } break;
    case napi_boolean: {
        bool result;
        auto status = napi_get_value_bool(env_, value, &result);
        this->MaybeThrowFromStatus(status, "Failed to retrieve boolean value");
        // KLOG_INFO("ArkTS") << "ArkTS::PrintValue: boolean - " << result;
    } break;
    case napi_number: {
        double result;
        auto status = napi_get_value_double(env_, value, &result);
        this->MaybeThrowFromStatus(status, "Failed to retrieve double value");
        // KLOG_INFO("ArkTS") << "ArkTS::PrintValue: number - " << result;
    } break;
    case napi_string: {
        size_t length = 0;
        napi_status status;
        ...
        status = napi_get_value_string_utf8(env_, value, reinterpret_cast<char *>(buffer.data()), length + 1, &length);
        this->MaybeThrowFromStatus(status, "Failed to get the string data");
        // KLOG_INFO("ArkTS") << "ArkTS::PrintValue: string - " << buffer;
    } break;
    case napi_symbol: {
        // KLOG_INFO("ArkTS") << "ArkTS::PrintValue: symbol";
    } break;
    case napi_object: {
        // KLOG_INFO("ArkTS") << "ArkTS::PrintValue: object";
    } break;
    case napi_function: {
        // KLOG_INFO("ArkTS") << "ArkTS::PrintValue: function";
    } break;
    case napi_external: {
        // KLOG_INFO("ArkTS") << "ArkTS::PrintValue: external";
    } break;
    case napi_bigint: {
        int64_t result;
        bool lossless;
        auto status = napi_get_value_bigint_int64(env_, value, &result, &lossless);
        this->MaybeThrowFromStatus(status, "Failed to retrieve int64 value");
        // KLOG_INFO("ArkTS") << "ArkTS::PrintValue: bigint - " << result;
    } break;
    default:
        break;
    }
...
bool ArkTS::IsNull(napi_value value) {
    return GetType(value) == napi_null;
}

bool ArkTS::IsUndefined(napi_value value) {
    return GetType(value) == napi_undefined;
}
