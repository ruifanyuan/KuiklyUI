...
/**
 * 宽松 JSON 扫描器，[JSONTokener] 各平台 actual 类的公共父类。
 *
 * 语义完全承袭 org.json：`=` / `=>` 作分隔符、`;` 等价 `,`、`/* */` `//` `#` 注释、
 * 单引号与无引号字面量、十六进制 / 八进制数、Int-vs-Long-vs-Double 选型、BOM 剥离等。
 * 平台原生解析器（Apple `NSJSONSerialization`、OHOS cJSON）只覆盖严格 JSON，
 * 遇到上述历史写法时都回退到这里，因此这份实现必须逐字节保持原有行为。
 *
 * 异常文案沿用 `message + this`，`this` 是具体平台的 [JSONTokener] 实例，
 * 保证各平台错误串与重构前完全一致。
 */
abstract class AbstractJSONTokener internal constructor(json: String) {
